    ...
    public class Sexo
    {
        ...
        public DataTable obtenerTodos()
        {
            try
            {
                ...
                dt = AccesoADatos.connectToDB.fillTableSQL("SELECT * FROM [MRFragancias].[dbo].[Sexo] WHERE Activo = 1");
                return dt;
            }
            catch (Exception)
            {
        ...
        public List<string> obtenerIdsPorNombre(List<string> listNombres)
        {
            try
            {
                ...
                DataTable dt = AccesoADatos.connectToDB.fillTableSQL("SELECT idFamiliaOlfativa FROM [MRFragancias].[dbo].[Sexo] WHERE Descripcion IN (" + arrayNombres + ")");
